...
def grab_vids(folder):
    ...
    :return: all videos (with .avi extension) in a given folder, EXCLUDING output files
    """
    vids = []
    for root, dirs, files in os.walk(folder):
        for f in files:
            if f.endswith('.avi') and not f.endswith('output.avi'):
...
def play_vid_t(M, h, w, index, scale, path, out, title="Video"):
    ...
    for i in range(M.shape[2]):
        ...
        out.write(frame)
        df, worms = worms_track(frame, min=100, max=1000, time=i, ratio = 1)
        df_l.append(df)
        ...
        cv2.imshow(title, cv2.addWeighted(b, 0.5, f, 0.8, 0))
