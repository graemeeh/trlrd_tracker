...
def log_abs(img, sigma: float = 0.3):
    ...
    :return: laplacian of gaussian of frame, highlights edges
    """
...
def median_frame(vid):
    ...
    frame_ids = [i for i in range(0, 41)]
    frames = []
    for f in frame_ids:
...
def mode_frame(vid, r):
    # This is from https://github.com/samwestby/OpenCV-Python-Tutorial/blob/main/8_background_est.py but made to grab the mode
    from scipy import stats
    frame_ids = [i for i in range(0, r)]
    frames = []
    for f in frame_ids:
        vid.set(cv2.CAP_PROP_POS_FRAMES, f)
        r, f = vid.read()
        if not r:
            ...
            exit()
        frames.append(f)
    return cv2.cvtColor(stats.mode(frames, axis=0).astype(np.uint8), cv2.COLOR_BGR2GRAY)

def resizr(image, scale):
    """
    replacement for scipy.misc.imresize
    :param image: input frame
    :param scale: how big relative to original the image should be
...
def feats_detect(vid, min: int = 10000, max: int = 10000000, ksize: int = 155):
    ...
    '''
    # circles = cv2.HoughCircles(blur_gray3, cv2.HOUGH_GRADIENT, 2, blur_gray3.shape[0]/10, param1=200, param2=600, minRadius=100, maxRadius= int(blur_gray3.shape[0]/1.5))
